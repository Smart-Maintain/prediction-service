...
class SensorReading(BaseModel):
    window: list[list[float]] = Field(
        ..., 
        description="A list of exactly 30 sequence cycles, where each sequence cycle contains exactly 18 normalized sensor features."
    )
...
def predict(data: SensorReading):
    ...
    for idx, cycle_data in enumerate(data.window):
        if len(cycle_data) != 18:
            raise HTTPException(status_code=400, detail=f"Each cycle must have exactly 18 features. Cycle {idx} has {len(cycle_data)}.")
